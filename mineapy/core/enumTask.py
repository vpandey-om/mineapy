...
class EnumerateTask:
    ...
    """
    def __init__(self, tfa_model, fba_model,params):
        """
        ...
        try:
            ...
            fva=self._get_blocked_reaction() # these are the blocked reaction
    ...
    def _get_blocked_reaction(self):
        ...
        if self.model_type=='fba':
            
            fva=flux_variability_analysis(self._fba_model,fraction_of_optimum=0)
        else:
            fva=variability_analysis(self._tfa_model)
    ...
    def _lump_one_per_bbb2(self, met_BBB, sink, force_solve):
        """
    ...
    def _lump_min_plus_p(self, met_BBB, sink, p, force_solve):
        ...
        with self._tfa_model as model:
            ...
            while len(lumps)<max_lumps:
                ...
                if self._param_dict['sum_flux']:
                    activated = [x for x in activation_vars
                               if abs(x.variable.primal) < epsilon]
                    expr = symbol_sum(activated)
                    model.add_constraint(kind=ForbiddenProfile,
                                        hook = model,
                                        ...
                                        expr = expr,
                                        lb = 0,
                                        ub = len(activated)-self._param_dict['diverge_min'],
                                        )
