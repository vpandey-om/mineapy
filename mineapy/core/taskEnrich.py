...
class TaskEnrichment():
    ...
    def fill_default_params(self):
        ...
        if "sum_flux" not in self.params:
            self.logger.info("Using default sum_flux : False")
            self.params["sum_flux"] = False
    ...
    def get_enrichment_table_for_de(self,mins,de_rxns,de_col='deregulated'):
        ...
        for k,v in mins.items():
            ...
            for idx,a_min_dict in enumerate(v):
                ...
                list_rxn_in_min_all.append(','.join(without_GPR))
        ...
        df['rxns in a min with GPR']=list_rxn_in_min
        df['rxns in a min']=list_rxn_in_min_all

        ## sort dataframe with p-values
        df=df[df[de_col+' rxns(%)']>.05]
    ...
    def run(self):
        ...
        mt_model = EnumerateTask(self._source_gem, self._gem,self.params) #### metabolic task model
